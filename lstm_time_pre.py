...
def data_pre(min):
    ...
    # 时间序列做为标签
    if(min==1):
        dataframe = pd.read_csv(r'E:\data_time_mul\data_wdz_notime.csv',usecols=[0,1,3],engine='python',index_col=0)
        # dateframe = dataframe[:24 * 60 * 28]
        train_size = 24 * 60 * 21  # 1分钟
        test_size = 24 * 60 * 28
    elif (min ==5):
        dataframe = pd.read_csv(r'E:\data_time_mul\data_wdz_notime_5min.csv',usecols=[0,1,3],engine='python',index_col=0)
        # dateframe = dataframe[:24 * 12 * 28]
        train_size = 24 * 12 * 21 # 5分钟
        test_size = 24 * 60 * 28
    elif (min ==10):
        dataframe = pd.read_csv(r'E:\data_time_mul\data_wdz_notime_10min.csv', usecols=[0, 1, 3], engine='python', index_col=0)
